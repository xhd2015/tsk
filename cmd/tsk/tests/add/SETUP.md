...
**Feature**: `tsk add` allocates id, writes task directory and index entry

```
# title + optional --topic / --parent / --label / --note flags
tsk add [--label L]... [--topic PATH | --parent ID] [--note T]... <title>
...
- Leaves set `req.Args` to the `add` invocation under test.
