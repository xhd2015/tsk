...
- **delete** — `delete <id>` permanently removes the task directory and `index/<id>`; nested sub-tasks require `--recursive` (whole subtree + all descendant indexes). Stdout `deleted <id>`. `--dry-run` uses the same plan (`PlanDelete`) and prints `[dry-run] would delete <id>  [<id>] <title>` per task (parent then descendants) without removing files; nested-without-recursive and unknown-id errors match live. No tombstone (task ids never reuse). Unlike `done`/`archive`, the task does not remain in the tree.
