...
name: tsk/create
description: >-
  Create inbox, topic-scoped, or nested sub-tasks with tsk create
...
```text
tsk create [--label LABEL]... [--topic PATH | --parent ID] [--note TEXT]... <title>
```
...
```bash
tsk create "inbox item"
tsk create --topic knowledge-base "report progress"
tsk create --topic 知识库 "via alias"          # stores canonical topic_path
tsk create --parent 6 "add more doctests to pricing"
tsk create --parent 9 "cover discount edge cases"
tsk create --label report --topic knowledge-base "weekly update"
tsk create --topic agent-pro \
...
- Prefer `--note` on `create` when context is known up front (dirs, grok
