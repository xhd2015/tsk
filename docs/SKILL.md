...
name: tsk
description: >-
  Task workflow CLI: create/list tasks under topics or nested parents,
...
`tsk create`, `tsk tree`, `tsk note`, and so on — this skill documents the
...
- Placement: inbox, under a topic path, or nested under a parent task via
  `--parent`.
- Sub-tasks: use `tsk create --parent <id>`; do not create a sibling under the
...
- Orient with `tsk tree` / `tsk tree --id N`; attach context with notes and
  progress (prefer `create --note` when creating; `note add` / `progress` later).
